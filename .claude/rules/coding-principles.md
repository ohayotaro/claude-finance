...
Every bot, script, or tool that operates on a strategy MUST:
1. Accept `strategy_id` as an explicit input (CLI arg, function parameter, or env var).
2. Resolve all paths (config, state, logs, reports) via `config/registry.toml` lookup -- never hardcode paths like `config/strategies/foo.toml`.
3. Reject unregistered `strategy_id` values with a clear error pointing the user to `/strategy-register`.
4. Tag all generated artifacts (logs, reports, saved files) with `strategy_id` and `logic_version`.

See `multi-strategy.md` section 7 for the full skill contract.
...
- Magic numbers: MagicNumber MUST be read from the per-strategy config file (populated by `/strategy-register`), MUST fall in the reserved range `20_000_000`-`89_999_999`, and MUST NOT be hardcoded in EA source. See `multi-strategy.md` section 3 for the allocation algorithm and account-type caveats
...
// Input parameters
input int    InpMagicNumber = 0;       // loaded from .set preset generated by /strategy-register
